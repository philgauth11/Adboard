...
def _sample_rows(leads=5):
    return [{
        ...
        "leads": leads, "purchases": 3, "revenue": 150.0, "roas": 6.0,
    }]
...
def test_upsert_metrics_stores_leads(app, db):
    c = _make_client(db)
    _upsert_metrics(c.id, "meta", "campaign", _sample_rows(leads=7))
    db.session.commit()
    m = AdMetric.query.filter_by(client_id=c.id).first()
    assert m.leads == 7
